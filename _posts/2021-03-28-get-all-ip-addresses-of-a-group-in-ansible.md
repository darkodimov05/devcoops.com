...
## ```join``` function
The ```join``` function is a filter that's allowing us to join a list of items into a string. Now, that we have IP addresses with the map function, we can concatenate them with a double quote:
```bash
